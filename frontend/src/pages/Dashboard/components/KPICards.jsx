...
export default function KPICards({ data }) {
  ...
  const kpis = [
    { 
      ...
      route: '/collections',
      trend: '+8%' // FIXED: static placeholder trend
    },
    { 
    ...
    <Grid container spacing={3}>
      {kpis.map((kpi, index) => (
        <Grid item xs={12} sm={6} md={3} key={index}>
          <Box onClick={() => navigate(kpi.route)} sx={{ cursor: 'pointer', height: '100%' }}>
            <Card sx={{ 
              height: '100%', 
              borderRadius: '16px',
              ...
              <CardContent sx={{ p: 3, '&:last-child': { pb: 3 } }}>
                ...
                {/* FIXED: Added trend badge */}
                {kpi.trend && (
                  <Box sx={{
                    display: 'inline-flex', alignItems: 'center', gap: 0.5,
                    px: 1, py: 0.25, borderRadius: '100px', mt: 0.5,
                    bgcolor: kpi.trend.startsWith('+') ? '#ECFDF5' : '#FFF1F2',
                    color: kpi.trend.startsWith('+') ? '#059669' : '#E11D48',
                    fontSize: '11px', fontWeight: 700
                  }}>
                    {kpi.trend.startsWith('+') ? '↑' : '↓'} {kpi.trend} vs yesterday
                  </Box>
                )}
